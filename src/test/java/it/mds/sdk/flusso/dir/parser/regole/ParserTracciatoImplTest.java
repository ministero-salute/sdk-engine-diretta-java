...
class ParserTracciatoImplTest {
    ...
    void integ() throws InterruptedException {
        ...
        RecordDtoDir copy = (RecordDtoDir) list.get(0);
    ...
    void integBLocco() {
        ...
        List<RecordDtoDir> r2 = list.stream().map(k -> (RecordDtoDir) k).collect(Collectors.toList());
    ...
    void scritturaNBlocchiSuXML() {
        ...
        List<RecordDtoDir> bloccoConv = blocco.stream().map(k -> (RecordDtoDir) k).collect(Collectors.toList());
        ...
        bloccoConv = blocco.stream().map(k -> (RecordDtoDir) k).collect(Collectors.toList());
